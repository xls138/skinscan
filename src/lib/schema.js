...
const MetricDetailSchema = z.object({
  ...
  percentile: z
    ...
    .min(0)
    .max(100)
    .describe("同龄人对比百分位，如87表示超过87%同龄人"),
  sub_items: z.object({
    item1: SubItemSchema,
    item2: SubItemSchema,
    item3: SubItemSchema,
...
export const SkinScanSchema = z.object({
  gender: z.enum(["female", "male"]).describe("性别，由AI自动判断"),
  ...
  tagline: z.string().max(50).describe("一句话评语，根据气质类型匹配风格"),
  ...
  concerns: z.array(z.string()).max(3).describe("检测到的问题点，用于制造悬念"),
});
